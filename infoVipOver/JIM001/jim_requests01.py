...
class Case01(unittest.TestCase):
    ...
    def test_case01(self):
        ...
        self.assertEqual(response01.status_code, 200, '返回状态错误，不为200')
        print(response01.text)
        Case01.token= jsonpath.jsonpath(response01.json(),'$..authToken')[0]   #获取token进行关联使用
        print( Case01.token,type(Case01.token))
